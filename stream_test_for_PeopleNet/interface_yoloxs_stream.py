...
#                     print(input_data['framedata']['meta']['source']['frame_count'] ,'not label')
                ...
                print('##',input_data['framedata']['meta']['source']['frame_count'], input_data['bbox'])
                res.append(input_data)
        return res 
    
#     def run_infer(self, input_data_batch, unavailable_routing_data_batch, reference_CM=None):
...
#         output_batch = self.postprocess(infer_result, scale_list, class_agnostic=True)
...
#         s3 = cuda.Stream(device="cuda")
        ev1 = self.s1.record_event()
        ev2 = self.s1.record_event()
...
# #         s3.wait_stream(s2)
#         s3.wait_stream(cuda.current_stream())
