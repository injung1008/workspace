...
def getFrameByVideo(video_path):
    ...
    print(f"cnt : {cnt}, frame_list: {len(frame_list)} frame loaded.")
...
video_path = '/DATA_17/ij/test/0001_compressed.mp4'
...
for i in range(len(frame_list)) : 
    print(f'frame_number : {i}')
    start = time.time()
    image = frame_list[i][..., ::-1]

    s1 = time.time()
    detection_out, keepCount_out = predict(image, model_w, model_h)
    print(f'predict_time : {time.time()-s1}')
...
#             print(f'class_id : {class_id}, xmin, ymin, w, h : {xmin, ymin, w, h}')
    ...
    print(f'inference_time : {time.time()-start}')

    
print(f'Total_inference_time : {time.time()- total_start}')
