...
if __name__ == "__main__":
    ...
    batch_size = 1
    trt_engine_path = 'model_ij_s_b128.trt'
...
#     exp = get_exp("/DATA_17/hjjo/YOLOX_pruning/YOLOX/exps/default/yolox_m.py",None)
    ...
    x = img_stack.astype(model.dtype)
    print(f"x.ravel() shape : {x.ravel().shape}")
    np.copyto(model.inputs[0].host,x.ravel())
    
    for inp in model.inputs:
        cuda.memcpy_htod_async(inp.device, inp.host, model.stream)
...
#         result1 = np.reshape(result,(batch_size,1,714000))
    ee = time.time()
    print(f"total_infer time : {ee - ss}")
    #결과값 텐서 변환 등 해주기
